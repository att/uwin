...
if $cmd no-rsa; then
  ...
  ssltest -v -bio_pair -tls1 -cert $cert2 -no_dhe -num 10 -f -time $extra || exit 1

  if $cmd no-dh; then
    echo skipping RSA+DHE tests
  else
    echo test tls1 with 1024bit RSA, 1024bit DHE, multiple handshakes
    ssltest -v -bio_pair -tls1 -cert $cert2 -dhe1024dsa -num 10 -f -time $extra || exit 1
  fi
fi
