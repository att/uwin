...
#ifndef lint
static	char sccsid[] = "@(#)print.c 1.1 90/03/23 SMI"; /* from S5R3 1.6 */
#endif
...
static int restrictnames = 1;		/* restrict termcap names */
...
int value;
{
    ...
    if (printing == pr_cap && restrictnames &&
...
char *value;
{
    ...
    if (printing == pr_cap)
        {
	if(restrictnames && !findcapname(capname, capstrs, ncapstrs))
            return;
	if (value)
