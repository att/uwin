...
fprintf(aha, "AHA#%d dp=%p:%s |stat|=%d |stat64|=%d\n", __LINE__, dp, dp, sizeof(struct stat), sizeof(struct stat64));
	if (stat(dp, &sb)) {
...
fprintf(aha, "AHA#%d dp=%p:%s path=%s\n", __LINE__, dp, dp, path);
	if ((fd = rcv_mktemp(sp, path, dp, S_IRWXU)) == -1)
		goto err;
	(void)close(fd);

	if ((ep->rcv_path = strdup(path)) == NULL) {
fprintf(aha, "AHA#%d path=%s strdup() failed\n", __LINE__, path);
