/**
 *
 * $Header: /cvsroot/lesstif/lesstif/lib/Xm-2.1/List.c,v 1.3 2005/03/22 14:48:04 dannybackx Exp $
 *
...
static const char rcsid[] = "$Header: /cvsroot/lesstif/lesstif/lib/Xm-2.1/List.c,v 1.3 2005/03/22 14:48:04 dannybackx Exp $";
...
_XmListSetRenderTable(Widget w, int o, XrmValue *v)
{
	...
	DEBUGOUT(_LtDebug(__FILE__, w, "_XmListSetRenderTable\n"));

	/* HACK
	 * this should be 0 or 1 (but it isn't).
	 * Assume that another value is bogus, replace it by 0.
	 */
	if ( ! ((lw->list.check_set_render_table == 0) ||
			(lw->list.check_set_render_table == 1))) {
		lw->list.check_set_render_table = 0;
		DEBUGOUT(_LtDebug(__FILE__, w, "_XmListSetRenderTable restore bogus\n"));
		_XmWarning(w, "_XmListSetRenderTable restore bogus\n");
	}
	if (lw->list.check_set_render_table) {
		/*
		 * This is the second time we're getting called.
		 * Therefore the result is undefined.
		 */
		v->addr = NULL;	/* FIX ME or should we pass nothing ? */
	} else {
		v->addr = (char *)&(lw->list.font);
		lw->list.check_set_render_table = True;
	}
}
