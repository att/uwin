...
struct mam_state mam;

/* Nonzero means don't run *any* commands,
   even + or $(MAKE) (-N).  */

int never_flag;

/* List of args given with -M switch.  */

static struct stringlist *mam_options = 0;
...
#ifdef MAM
    N_("\
  -F, --force                 Equivalent to -B, --always-make.\n"),
#endif /*MAM*/
    N_("\
...
#ifdef MAM
    N_("\
  -M TYPE[:FILE[:[LABEL][:ROOT]]], ----mam=TYPE[:FILE[:[LABEL][:ROOT]]]\n\
                              Generate MAM (make abstract machine) code in FILE.\n"),
#endif /*MAM*/
    N_("\
...
#ifdef MAM
    N_("\
  -N, --really-just-print, --never\n\
                              Don't actually run *any* commands, really.\n"),
#endif /*MAM*/
    N_("\
...
#ifdef MAM
    { 'F', flag, (char *) &always_make_flag, 1, 1, 0, 0, 0, "force" },
#endif /*MAM*/
...
#ifdef MAM
    { 'M', string, (char *) &mam_options, 1, 1, 0, 0, 0, "mam" },
#endif /*MAM*/
    { 'n', flag, (char *) &just_print_flag, 1, 1, 1, 0, 0, "just-print" },
#ifdef MAM
    { 'N', flag, (char *) &never_flag, 1, 1, 0, 0, 0, "really-just-print" },
#endif /*MAM*/
...
#define	FATAL_SIG(sig)							      \
  if ((RETSIGTYPE (*) ()) bsd_signal (sig, fatal_error_signal) == SIG_IGN)    \
  ...
  if (mam_options)
    {
      ...
      mam.pwd = starting_directory;
      mam.pwdlen = strlen (mam.pwd);
      mam.label = "";
      p = mam_options->list;
      s = *p;
      type = s = savestring (s, strlen (s));
      if (t = strchr(s, ':'))
	*t++ = 0;
      if (o = strchr(s, ','))
	*o++ = 0;
      if (*s == *(mam.type = "dynamic") &&
	  (!*(s + 1) || !strcmp (s, mam.type)))
	mam.dynamic = 1;
      else if (*s == *(mam.type = "regress") &&
	       (!*(s + 1) || !strcmp (s, mam.type)))
        {
          mam.regress = 1;
          silent_flag = 1;
        }
      else if (*s == *(mam.type = "static") &&
	       (!*(s + 1) || !strcmp (s, mam.type)))
	mam.statix = 1;
      else
	fatal (NILF, _("Invalid MAM type `%s': {dynamic,regress,static} expected"), s);
      while (s = o)
        {
          if (o = strchr(s, ','))
	    *o++ = 0;
	    ...
	    fatal (NILF, _("Invalid MAM option `%s': [no]{dontcare,port} expected"), s);
        }
      if (t)
        {
	  s = t;
	  if (t = strchr (s, ':'))
	    {
	      *t++ = 0;
	      if (isdigit (*t))
	        {
	          while (isdigit (*t))
	            mam.parent = mam.parent * 10 + *t++ - '0';
		  if (!mam.regress)
                    {
	              mam.label = (char *) xmalloc (7);
	              sprintf (mam.label, "%05d ", getpid ());
	            }
	        }
              if (*t != '\0')
	        {
	          if (*t != ':')
                    fatal (NILF, _("%s: MAM label expected"), t);
	          t++;
	        }
	      if (*t == '\0')
	        t = 0;
	        ...
	        {
		  char *x;
                  mam.rootlen = strlen (t);
	          if (*t != '/')
	            mam.rootlen += mam.pwdlen + 1;
	          x = (char *) xmalloc (mam.rootlen + 1);
	          sprintf (x, "%s%s%s",
		           *t == '/' ? "" : mam.pwd,
		           *t == '/' ? "" : "/",
		           t);
      ...
      if (!mam.root)
	{
      ...
      else if (!strcmp (s, "/dev/fd/2") ||
	       !strcmp (s, "/dev/stderr"))
        mam.out = stderr;
      else
	{
	  if (!mam.parent && (mam.out = fopen (s, "w")))
	    fclose (mam.out);
	  if (!(mam.out = fopen (s, "a")))
	    fatal (NILF, "Cannot write MAM output file `%s'", s);
          /* Make sure mam.out is line-buffered.  */
#ifdef	HAVE_SETLINEBUF
          setlinebuf (mam.out);
#else   /* setlinebuf missing.  */
#ifndef	SETVBUF_REVERSED
          setvbuf (mam.out, (char *) 0, _IOLBF, BUFSIZ);
#else	/* setvbuf not reversed.  */
          /* Some buggy systems lose if we pass 0.  */
          setvbuf (mam.out, _IOLBF, xmalloc (BUFSIZ), BUFSIZ);
#endif	/* setvbuf reversed.  */
#endif	/* setlinebuf missing.  */
	}
      p[1] = 0;
      *p = (char *) xmalloc (strlen (type) +
		    strlen (s) +
		    (*s == '/' ? 0 : mam.pwdlen) +
		    mam.rootlen +
		    10);
      sprintf (*p, "%s:%s%s%s:%d:%s",
	       type,
	       *s == '/' ? "" : mam.pwd,
	       *s == '/' ? "" : "/",
	       s,
      ...
      free (type);
      if (mam.out == stdout)
        inhibit_print_directory_flag = 1;

      /* The first MAM output.  */
      if (mam.regress)
	{
          fprintf (mam.out, "%sinfo mam %s %05d\n",
	           mam.label, mam.type, mam.parent);
	  fprintf (mam.out, "%sinfo start regression\n", mam.label);
	}
      else
	{
          fprintf (mam.out, "%sinfo mam %s %05d 1994-07-17 GNU make version %s%s%s\n",
	           mam.label, mam.type, mam.parent, version_string,
  	           remote_description != 0 && *remote_description != '\0' ? "-" : "",
  	           remote_description != 0 && *remote_description != '\0' ? remote_description : "");
          if (!mam.statix || *mam.label)
	    {
	      int f;
              f = warn_undefined_variables_flag;
              warn_undefined_variables_flag = 0;
	      fprintf (mam.out, "%sinfo start %lu\n",
		       mam.label, (unsigned long) time ((time_t *)0));
	      fprintf (mam.out, "%sinfo pwd %s %s\n",
		       mam.label, mam.root, mamname (enter_file (mam.pwd)));
	      s = variable_expand ("$(strip $(VPATH))");
	      if (*s != '\0')
	        fprintf (mam.out, "%sinfo view %s\n", mam.label, s);
              warn_undefined_variables_flag = f;
	    }
	}
    }
#endif /*MAM*/
...
#define	ADD_FLAG(ARG, LEN) \
  ...
  /* Loop twice so pure flags are bundled in one group. */
  for (cs = switches; cs->c != '\0'; ++cs)
  ...
  for (cs = switches; cs->c != '\0'; ++cs)
#ifdef MAM
    if (cs->toenv && (!makefile || !cs->no_makefile || cs->c == 'n' && mam.out))
#else
    if (cs->toenv && (!makefile || !cs->no_makefile))
#endif /*MAM*/
      switch (cs->type)
...
print_version (void)
{
  ...
  printf ("%sGNU Make %s\n\
%sCopyright (C) 2004  Free Software Foundation, Inc.\n",
...
die (int status)
{
  ...
  if (!dying)
	...
	{
	  if (mam.regress)
	    fprintf (mam.out, "%sinfo finish regression\n", mam.label);
	  else if (mam.dynamic || *mam.label != '\0')
	    fprintf (mam.out, "%sinfo finish %lu %d\n", mam.label,
		     (unsigned long) time ((time_t *)0), status);
	}
#endif /*MAM*/
...
#ifdef MAM

/* Return MAM name for file.  */

char*
mamname (struct file *f)
{
  register char *s, *r, *u, *v;
  int n;
  static char *buf, *ret;
  static int len;
  s = f->name;
  if (f->phony || !strchr (s, '/') && access (s, 0))
    return s;
  pathcanon (s);
  if (!mam.root)
      return s;
  if (*s == '/' && *(s + 1) == 'u' && !strncmp (s, "/usr/", 5))
    {
      r = s + 5;
      switch (*r)
	{
	...
	case 'l':
	  ...
	  break;
	case 'i':
	  if (!strncmp (r, "include/", 8))
	    return s;
	  break;
	}
    }
  n = strlen (s) + mam.pwdlen + 2;
  if (n > len)
    {
      if (buf)
	{
