...
#ifdef MAM
	      if (never_flag || mam.statix || mam.regress)
	        just_print_flag = n;
#endif /*MAM*/
...
update_file (struct file *file, unsigned int depth)
{
  ...
  if (f->considered == considered)
	...
	{
          file->done = 1;
          fprintf (mam.out, "%sprev %s\n", mam.label, mamname (file));
        }
#endif /*MAM*/
...
#ifdef MAM
  if (mam.out && (file->updated || file->done))
    {
      file->done = 1;
      fprintf (mam.out, "%sprev %s\n", mam.label, mamname (file));
    }
...
#ifdef MAM
  must_make = always_make_flag && file->deps || noexist;
#else
  must_make = noexist;
#endif /*MAM*/
...
#ifdef MAM
      if (mam.out && !file->done)
        {
          file->done = 1;
          fprintf (mam.out, "%sdone %s%s\n", mam.label, mamname (file), (file->phony || !just_print_flag && access (file->name, 0)) ? " virtual" : "");
        }
#endif /*MAM*/
...
#ifdef MAM
      d->changed |= always_make_flag && file->deps || noexist || d_mtime > this_mtime;
#else
...
#ifdef MAM
              if (mam.out && (mam.dynamic || mam.regress))
                fprintf (mam.out, "%scode %s %d %lu %lu%s\n", mam.label,
    		         mamname (d->file), file->update_status,
			 mam.dynamic ? d->file->last_mtime : 0L,
			 mam.dynamic ? time ((time_t *) 0) : 0L,
			 d->file->last_mtime == d->file->mtime_before_update
			   ? " same" : "");
#endif /*MAM*/
	    }
        }
#ifdef MAM
      if (!file->phony && mam.out && (mam.dynamic || mam.regress))
        fprintf (mam.out, "%scode %s %d %lu %lu%s\n", mam.label,
	         mamname (file), file->update_status,
		 mam.dynamic ? file->last_mtime : 0L,
		 mam.dynamic ? time ((time_t *) 0) : 0L,
...
remake_file (struct file *file)
{
  if (file->cmds == 0)
	...
	file->update_status = 0;
#ifdef MAM
      else if (file->is_target || always_make_flag && file->deps)
#else
...
#ifdef MAM
      if (mam.dynamic || mam.regress)
        fprintf (mam.out, "%sbind %s %lu\n", mam.label, mamname (file), mam.dynamic ? mtime : 0L);
#endif /*MAM*/
    }
  else
#endif
              ...
              if (gpath_search (name, strlen(name) - strlen(file->name) - 1))
                {
                  ...
                  mtime = file_mtime (file);
                  if (mam.dynamic || mam.regress)
                    fprintf (mam.out, "%sbind %s %lu %s\n", mam.label, mamname (file), mam.dynamic ? mtime : 0L, name);
                  return mtime;
#else
...
#ifdef MAM
      else if (mam.dynamic || mam.regress)
        fprintf (mam.out, "%sbind %s %lu\n", mam.label, mamname (file), mam.dynamic ? mtime : 0L);
#endif /*MAM*/
    }
	...
	if (adjusted_now < adjusted_mtime)
	  {
		...
		error (NILF, _("Warning: File `%s' has modification time in the future"),
                       file->name);
#else
		/* (long) cast is msvc workaround */
		double from_now =
		  (long)(FILE_TIMESTAMP_S (mtime) - FILE_TIMESTAMP_S (now))
		   + (long)(FILE_TIMESTAMP_NS (mtime) - FILE_TIMESTAMP_NS (now))
		      / 1e9;
