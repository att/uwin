...
#endif
{
  ...
  if (fmt != 0)
    {
#ifdef MAM
      if (mam.out && fileno(mam.out) == fileno(stdout))
        printf ("%sinfo mam message ", mam.label);
      else
#endif /* MAM */
      if (prefix)
	{
...
#ifdef MAM
  if (mam.out && (!mam.statix || *mam.label))
    {
      fprintf (mam.out, "%sinfo mam warning ", mam.label);
      VA_START(args, fmt);
      VA_PRINTF (mam.out, fmt, args);
      VA_END (args);
      putc ('\n', mam.out);
    }
#endif /*MAM*/
...
#endif
{
  ...
  if (mam.out && (!mam.statix || *mam.label))
    {
      fprintf (mam.out, "%sinfo mam error ", mam.label);
      VA_START(args, fmt);
      VA_PRINTF (mam.out, fmt, args);
      VA_END (args);
      putc ('\n', mam.out);
