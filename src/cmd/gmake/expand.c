...
recursively_expand_for_file (struct variable *v, struct file *file)
{
  ...
  v->expanding = 0;
#ifdef MAM
  if (mam.parameterize && v->exportable && !v->special)
    {
	if (!v->setv || automatics != mam.automatics)
	  {
            if (!v->append && !v->per_target)
	      v->setv = 1;
            fprintf (mam.out, "%ssetv %s %s\n", mam.label, v->name, value);
	  }
	free(value);
        value = (char *) xmalloc (strlen (v->name) + 4);
	sprintf (value, "${%s}", v->name);
    }
#endif /*MAM*/
