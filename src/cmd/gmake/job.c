...
#ifdef MAM
  if (mam.out && (!mam.statix || !never_flag && (just_print_flag || !(flags & COMMANDS_RECURSE))))
    {
      char *b, *e, *f;
      f = mamname (child->file);
      if (mam.dynamic || mam.regress)
        fprintf (mam.out, "%sinit %s %lu\n", mam.label,
	         f, mam.dynamic ? time ((time_t *) 0) : 0L);
      for (b = p; e = strchr (b, '\n'); b = e + 1)
	{
	  fprintf (mam.out, "%sexec %s %-.*s\n", mam.label, f, e - b, b);
	  f = "-";
	}
      fprintf (mam.out, "%sexec %s %s\n", mam.label, f, b);
    }
...
#ifdef MAM
  /* Only actions that will not execute are parameterized. */
  if (mam.statix)
    {
      if (never_flag)
	mam.parameterize = 1;
      else
	{
          ...
          for (i = 0; i < cmds->ncommand_lines; ++i)
    	    {
              for (p = cmds->command_lines[i]; isspace(*p); p++);
              if (*p != '+' && (len = strlen (p))
