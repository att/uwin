...
#ifdef MAM
    unsigned int done:1;	/* MAM trace done */
#endif /*MAM*/
  };
...
struct mam_state			/* mam state			*/
{
	...
	int		parameterize;	/* parameterize MAM exec action	*/
	int		parent;		/* mam parent label		*/
	int		pwdlen;		/* strlen (mam.pwd)		*/
	int		rootlen;	/* strlen (mam.root)		*/

	unsigned long	automatics;	/* automatic expand capability	*/ 
...
extern char *mamname PARAMS ((struct file *));
