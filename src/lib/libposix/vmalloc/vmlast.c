...
#endif
{
	...
	if(!vd->free || addr < (Void_t*)vd->free || addr >= (Void_t*)vd->seg->baddr)
		return -1L;
	else	return (long)((Vmuchar_t*)addr - (Vmuchar_t*)vd->free);
}
...
#endif
{
	...
	if(!vd->free || addr != (Void_t*)vd->free )
		...
		return (long)((Vmuchar_t*)vd->seg->free - (Vmuchar_t*)addr);
	else	return (long)((Vmuchar_t*)vd->seg->baddr - (Vmuchar_t*)addr - sizeof(Head_t));
}
