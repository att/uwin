/************************************************************************
 * Version 1.1
 * Modified by	: Nilendu Pal (Wipro Infotech)
 *
 * Purpose of Modification:	
 * 		The assign the following rights to the USER on the Local System
 * 		1.	SeServiceLogonRight			(Logon as a Service)
 * 		2.	SeIncreaseQuotaPrivilege 	(Increased Quota)
 * 		3.	SeCreateTokenPrivilege   	(Create Token Object>
 * 		4. 	SeTcbPrivilege				(Act as part of OS)
 * 		5.	SeAssignPrimaryTokenPrivilege (Replace Process Level Token)
 *
 *********************************************************************/
    ...
    )
{
    ...
    PSID pSid;
	DWORD PrivilegeLevel=5;			// By default, give all 5 Privileges

    NTSTATUS Status;
    int iRetVal=RTN_ERROR;          // assume error from main

    if(argc == 1) {
        fprintf(stderr,"Usage: %s Account [PrivilegeLevel]\n", argv[0]);
        return RTN_USAGE;
    }
...
/********************************************************
 * We are using the Secong Argument to read in the Privilege Level
 * to be assigned to the User
 * 			-Nilendu
 ********************************************************/
    if(argc == 3) 
		PrivilegeLevel = atoi(argv[2]);
	if(PrivilegeLevel > 5) // We only have 5 PrivilegeLevels
		PrivilegeLevel = 5;
